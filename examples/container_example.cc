...
static uint64_t parse_run_ycsb(FILE *fp, struct hash_test_ycsb_ops *oplist, uint64_t max_ops, 
                                int blade_id, int num_nodes, int num_threads)
{
    uint64_t run_op_idx = 0, run_read_op_idx = 0, rec_op_idx=0, run_write_op_idx=0;
    if (!fp || !oplist)
    {
        return 0;
    }
    ...
    ssize_t read = 0;
    unsigned int num_workers = num_nodes * num_threads;
    unsigned int next_worker_idx = 0;
    unsigned int start_op_idx = blade_id * num_threads;
    long start_file_loc = ftell(fp);

    for (int thread_id = 0; thread_id < num_threads; thread_id++)
    {
        int global_th_id = start_op_idx + thread_id;
        fseek(fp, start_file_loc, SEEK_SET);
        run_op_idx = 0;
        run_read_op_idx = 0;
        run_write_op_idx = 0;
        while (run_op_idx < max_ops)
        {
            read = getline(&line, &len, fp);
            if (read == -1)
                break;  // end of the file
            
            if (line[0] == 'R' || line[0] == 'U')
            {
                char ops[16] = {0};
                char dummy_table[16] = {0};
                uint64_t key_val;
                char dummy_val[MAX_VALUE_SIZE] = {0};
                if (strncmp(line, "READ", 4) == 0)
                {
                    if (run_read_op_idx % num_workers == global_th_id)
                    {
                        sscanf(line, "%s %s user%lu ", ops, dummy_table, &key_val);
                        oplist[rec_op_idx].key = key_val;
                        memset(&oplist[rec_op_idx].value, 0, sizeof(oplist[rec_op_idx].value));
                        oplist[rec_op_idx].opcode = YCSB_READ;
                        rec_op_idx ++;
                    }
                    run_read_op_idx ++;
                ...
                {
                    sscanf(line, "%s %s user%lu [ field0=%1000c", ops, dummy_table, &key_val, dummy_val);
                    if (is_write_rec_for_thread(run_write_op_idx, key_val, blade_id, num_nodes, thread_id, num_threads)) 
                    {
                        memcpy(oplist[rec_op_idx].value, dummy_val, MAX_VALUE_SIZE);
                        oplist[rec_op_idx].key = key_val;
                        oplist[rec_op_idx].opcode = YCSB_UPDATE;
                        rec_op_idx ++;
                    }
                    run_write_op_idx ++;
                }
    ...
    std::printf("Total run operations: %lu\n", rec_op_idx);
    return rec_op_idx;
}


struct hash_test_ycsb_ops *get_oplist_from_run_dat(char *run_file, int blade_id, int num_nodes, int num_threads, int *num_total_run_ops)
{
    FILE *fp_ycsb_run = std::fopen(run_file, "r");
    if (!fp_ycsb_run) {
...
static void *run_rocksdb_ycsb(void *args)
{
    ...
    if (thread_id == 0) {
        ...
        // map oplist file
        int num_total_run_ops;
        oplist = get_oplist_from_run_dat(run_file, blade_id, num_node, num_thread_per_blade, &num_total_run_ops);
        // exit(-1);
        /*
        ...
        */
        ...
        num_op = t_args->num_op = num_total_run_ops / num_thread_tot;
...
static int launch_workers(int num_node, int num_thread, char *load_file, char *run_file) {
    ...
    printf("launch_workers: load file path: %s\n", load_file);
    ...
    for (int tid = 0; tid < num_thread; ++tid) {
        for (int nid = 0; nid < num_node; ++nid) {
            ...
            printf("thread_args[i]: load file path: %s\n", thread_args[i].load_file);
